...
> **端口避让**：缺省端口 18080；**被占用时自动 +1 顺延**（18080 → 18081 → …，最多顺延
> `server.portShiftLimit` 个，默认 100），多个以本框架为 core 的项目同时启动互不冲突——
> 实际端口以启动日志 `[server] 监听 http://…` 为准（管理页 `/manage/api/status` 也显示实际端口）。
> 不需要避让可关：`"server": { "portAutoShift": false }`；想用系统随机端口写 `"port": 0`。